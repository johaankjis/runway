...
export function DocumentsView() {
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    setNotice(
      names.length
        ? `Received ${names.join(", ")}. Upload ingestion is not part of this milestone, so nothing was stored. Documents shown below come from the deterministic fixture.`
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Documents"
        subtitle="Upload, manage, and see what Runway has extracted into signals."
        actions={
          <Button onClick={() => inputRef.current?.click()} icon={<Upload className="h-4 w-4" aria-hidden />}>
            Upload document
          </Button>
        }
      />
